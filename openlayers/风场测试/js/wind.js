	...
	if (datav && datau) {
		...
		for (var x = 0; x < w; x++) {
			...
			for (var y = 0; y < h; y++) {
				...
				var ux = x / (w - 1);
				var uy = y / (h - 1);
				var lon =  this.x0 * (1 - ux) + this.x1 * ux;
				var lat =  this.y0 * (1 - uy) + this.y1 * uy;
				var m = Math.PI * lat / 180;
				var length = v.length();
				if (length) {
				    total += length * m;
		...
		var result = new VectorField(field, this.x0,this.y0,this.x1, this.y1);
	  // window.console.log('total = ' + total);
		// window.console.log('weight = ' + weight);
	  if (total && weight) {
...
// VectorField.readImgArr = function(imageArr,extent) {
// 		var field = [];
// 		var w = 720;
// 		var h = 360;
// 		var n = 2 * w * h;
...
// 		//extent=extent||[73, 4, 135, 53];
// 		extent=[-179.5, 89.5, 179.5, -89.5];
...
// 				var ux = x / (w - 1);
// 				var uy = y / (h - 1);
// 				var lon =  extent[0] * (1 - ux) + extent[2] * ux;
// 				var lat =  extent[1] * (1 - uy) + extent[2] * uy;
// 				var m = Math.PI * lat / 180;
// 				var length = v.length();
// 				if (length) {
// 				    total += length * m;
...
// 			}
// 		}
// 		var result = new VectorField(field, extent[0], extent[1], extent[2], extent[3]);
// 	  // window.console.log('total = ' + total);
// 		// window.console.log('weight = ' + weight);
// 	  if (total && weight) {
...
var MotionDisplay = function(canvas, field, numParticles,extent) {
	this.canvas = canvas;
    this.field = field;
	this.numParticles = numParticles||3677.725;
	this.first = true;
	this.maxLength = field.maxLength;
	this.extent=extent; //范围，左上和
	this.speedScale = 1;
...
MotionDisplay.prototype.startMove  = function() {
	...
	var ext2 = [73, 4, 135, 53];
	var zoom=monitorMap.getView().getZoom();
	if(zoom>4){
		ext2=monitorMap.getView().calculateExtent(mapSize);
	}else{
		ext2=[73, 4, 135, 53]
	}
...
MotionDisplay.prototype.moveThings = function() {
	...
	var speed = 0.005*this.moveSpeed*resolution;
...
MotionDisplay.prototype.draw = function() {
	...
	for (var i = 0; i < this.particles.length; i++) {
		...
		proj = monitorMap.getPixelFromCoordinate([p.x, p.y]);
